...
<!-- 全部已上架供应商 -->
    ...
    <div class="dcanmou_so_area" style="margin-left:30px;">
      ...
      <div class="inner">
                <div class="actions_area">
                    ...
                    <span class="txt">自动上架已选供应商商品</span>
                    ...
                    <span class="dcanmou_newUpload_time">最新上传时间：<span>{{data.new_put_time}}</span></span> <!--这个为全局样式-->
                </div>
                <!-- 表格 -->
                <Table border :columns="Shelfcolumns" :data="data.shops" @on-selection-change="selectMore"></Table>
                <div class="pageBar">
                    <Page :current="data.current"  :total="data.total" simple @on-change="providerPageChange"></Page>
                </div>
            </div>
            <!-- 改价 -->
 ...
 <!-- end -->
    ...
    <div class="dcm_price_info" id="dcm_price_info">
        ...
        <p  v-if="hoverData.all_change2.length!=0">
            <p v-for="(item, $index) in hoverData.all_change2" :key="$index" style="padding-left:10px">加价:<span style="padding-left:10px">{{item.val}}{{item.typeName}}</span>
            </p>
        </p>
        <p v-if="hoverData.all_change2.length==0"><span>暂无加价</span></p> 
        
        <p style="padding-left:5px">区间内:</p>
        <p v-if="hoverData.info_change.length!=0">
        ...
        <p v-if="hoverData.info_change.length==0"><span>暂无加价</span></p>
    </div>
...
export default {
   ...
   methods: {
       ...
       ...mapMutations({showGoodsDeatail: 'openGoodsDetail'}), // 显示店铺详情
