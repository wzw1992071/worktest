    ...
    MobileArea.prototype = {
        ...
        bindEvent: function() {
            ...
            function popupString(e) {
                ...
                _self.gearArea.innerHTML = '<div class="area_ctrl slideInUp">' +
                    ...
                    '<div class="area_btn larea_center">' + _self.centerTitle + '</div>' +
            ...
            function areaCtrlInit() {
                try { _self.gearArea.querySelector(".area_province").setAttribute("val", _self.value[0]) } catch (err) {};
                try { _self.gearArea.querySelector(".area_city").setAttribute("val", _self.value[1]) } catch (err) {};
                try { _self.gearArea.querySelector(".area_county").setAttribute("val", _self.value[2]) } catch (err) {};
            ...
            _self.getData(function() {
                if (!_self.trigger) return;
                if (_self.type === 'string') {
                    if (_self.gearArea && document.body.contains(_self.gearArea)) document.body.removeChild(_self.gearArea);
                    // _self.trigger.removeEventListener("click", popupString);

                    // _self.trigger.addEventListener('click', popupString);
                    // params.trigger
                    // var $events = $(_self.params.trigger).data("events");
                    ...
                    // } else {
                    //     // 未绑定
                    //     console.log('meibangding');
                    // }
                    $(_self.params.trigger).on('click', popupString);

                } else {
                    if (_self.gearArea) document.body.removeChild(_self.gearArea);
                    _self.trigger.removeEventListener("click", popupArea);
        ...
        finish: function(e) {
            ...
            var provinceVal = '',
                ...
                countyText = '';
            if (area_province) {
                provinceVal = parseInt(area_province.getAttribute("val"));
                provinceText = area_province.childNodes[provinceVal].textContent;
            }
            if (area_city) {
            ...
            if (this.callback) this.callback(_self.value);
            _self.close(e);
        },
        close: function(e) {
            e.preventDefault();
            ...
            document.body.contains(_self.gearArea) && document.body.removeChild(_self.gearArea);
        }
    }
    return MobileArea;
})()